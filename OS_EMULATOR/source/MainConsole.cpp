...
void MainConsole::showVMStat() {
    ...
    int r = scheduler->getRunnableProcessCount();
    int b = scheduler->getBlockedProcessCount();
    size_t total_mem = scheduler->getTotalMemory();
    size_t used_mem = scheduler->getUsedMemory();
    size_t free_mem = scheduler->getFreeMemory();
    ...
    size_t pgin = scheduler->getPagesPagedIn();
    size_t pgout = scheduler->getPagesPagedOut();
    ...
    this->outputList.push_back("|---procs--|----memory(bytes)----|--------cpu-ticks-------|----paging----|");
